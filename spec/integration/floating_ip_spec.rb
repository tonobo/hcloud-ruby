...
describe 'FloatingIP', :integration, integration_helper: :server do
  ...
  it '#[] -> find by id' do
    ...
    expect(client.floating_ips[id].home_location).to be_a Hcloud::Location
  ...
  it "#update(description: 'moo')" do
    ...
    expect { client.floating_ips.find(@ids[0]).update(description: 'moo') }.not_to raise_error
    expect(client.floating_ips.find(@ids[0]).description).to eq('moo')
  end
