...
describe 'Image', :integration, integration_helper: :server do
  ...
  before(:context) do
    # let's collect all the images we create, because we cannot easily access them by
    # their description
    @images = []
  end

  before(:each) do
    # use the first of our newly created images when we need for perform an action
    # on an image
    @image_id = @images[0]
  end
  ...
  it '#update(description:) - handle nil' do
    expect { client.images[@image_id].update(description: nil) }.to(
  ...
  it '#update(type:) - handle nil' do
    expect { client.images[@image_id].update(type: nil) }.to(
      raise_error(Hcloud::Error::InvalidInput)
    )
  end
  ...
  it '#where(name:) -> handle nonexistent' do
    expect(client.images.where(name: nonexistent_name).count).to eq(0)
  end
